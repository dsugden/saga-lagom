...
/**
  * This is an event sourced entity. It has a state, [[SagalagomState]], which
  ...
  */
class SagalagomEntity extends PersistentEntity {

  override type Command = SagalagomCommand[_]
  override type Event = SagalagomEvent
  override type State = SagalagomState

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  override def initialState: SagalagomState = SagalagomState("Hello", LocalDateTime.now.toString)
  ...
  override def behavior: Behavior = {
    case SagalagomState(message, _) => Actions().onCommand[UseGreetingMessage, Done] {
      ...
      case (GreetingMessageChanged(newMessage), state) =>
        ...
        // the event.
        SagalagomState(newMessage, LocalDateTime.now().toString)
...
/**
  ...
  */
case class SagalagomState(message: String, timestamp: String)

object SagalagomState {
  /**
    ...
    */
  implicit val format: Format[SagalagomState] = Json.format
}

/**
  * This interface defines all the events that the SagalagomEntity supports.
  */
sealed trait SagalagomEvent

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends SagalagomEvent
...
/**
  ...
  */
sealed trait SagalagomCommand[R] extends ReplyType[R]
...
/**
  ...
  */
case class UseGreetingMessage(message: String) extends SagalagomCommand[Done]
...
/**
  ...
  * person.
  */
case class Hello(name: String, organization: Option[String]) extends SagalagomCommand[String]
...
/**
  ...
  * application loader.
  */
object SagalagomSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    JsonSerializer[UseGreetingMessage],
    JsonSerializer[Hello],
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[SagalagomState]
  )
}
